...
<div class="isikanan"><!--Awal class isi kanan-->
		...
		<table class="tabel" id="results">
			...
			$no=1;
			if($_SESSION['leveluser']=='Super Admin'){
			$databerita=mysql_query("SELECT * FROM sh_berita,sh_users, sh_kategori WHERE sh_berita.s_username=sh_users.s_username AND sh_berita.id_kategori=sh_kategori.id_kategori AND sh_berita.s_username='$_POST[penulis]' ORDER BY id_berita DESC");
			}
			...
			<tr class="tabel"><td class="tabel" colspan="8"><b>DATA BERITA DENGAN PENULIS <u><?php
			$nama_penulis_berita=mysql_query("SELECT * FROM sh_users WHERE s_username='$_POST[penulis]'");
			$npb=mysql_fetch_array($nama_penulis_berita);
			echo "$npb[nama_lengkap_users]";
			?></u> TIDAK DITEMUKAN</b></td></tr>
			<?php }
			?>
