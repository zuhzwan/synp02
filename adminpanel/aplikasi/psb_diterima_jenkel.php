...
<div class="isikanan"><!--Awal class isi kanan-->
		...
		<table class="tabel" id="results">
			...
			<?php
			$no=1;
			$psb=mysql_query("SELECT * FROM sh_psb WHERE status_psb='1' AND jenkel='$_GET[kode]' ORDER BY id_psb DESC");
			...
			<tr class="tabel">
				...
				<td class="tabel">
					<?php if ($_GET[kode]=='L'){ ?>
					<div class="hapusdata"><a href="<?php echo"$database?pilih=laki&untukdi=hapus&id=$p[id_psb]";?>">hapus</a></div>
					<div class="editdata"><a href="<?php echo"?pilih=edit&id=$p[id_psb]";?>">edit</a></div>
					<?php if ($p[status_psb]==0){ ?>
					<div class="terbitdata"><a href="<?php echo"$database?pilih=laki&untukdi=terima&id=$p[id_psb]";?>">terima</a></div>
					<?php }
					else { ?>
					<div class="bataldata"><a href="<?php echo"$database?pilih=laki&untukdi=tolak&id=$p[id_psb]";?>">tolak</a></div>
					<?php } }
					
					else {?>
					<div class="hapusdata"><a href="<?php echo"$database?pilih=perempuan&untukdi=hapus&id=$p[id_psb]";?>">hapus</a></div>
					<div class="editdata"><a href="<?php echo"?pilih=edit&id=$p[id_psb]";?>">edit</a></div>
					<?php if ($p[status_psb]==0){ ?>
					<div class="terbitdata"><a href="<?php echo"$database?pilih=perempuan&untukdi=terima&id=$p[id_psb]";?>">terima</a></div>
					<?php }
					else { ?>
					<div class="bataldata"><a href="<?php echo"$database?pilih=perempuan&untukdi=tolak&id=$p[id_psb]";?>">tolak</a></div>
					<?php }} ?>
				</td>
			</tr>
			<?php
