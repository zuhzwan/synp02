...
<div class="isikanan"><!--Awal class isi kanan-->
		...
		<div class="atastabel" style="margin-bottom: 10px">
			...
			<?php echo "<form method='POST' action='$database?pilih=terbit&untukdi=hapusbanyak'>"; ?>
			<div class="cari">
		...
		<table class="tabel" id="results">
			...
			$no=1;
			if($_SESSION['leveluser']=='Super Admin'){
			$databerita=mysql_query("SELECT * FROM sh_berita,sh_users, sh_kategori WHERE sh_berita.s_username=sh_users.s_username AND sh_berita.id_kategori=sh_kategori.id_kategori AND status_terbit='1' ORDER BY id_berita DESC");
			}
			else {
			$databerita=mysql_query("SELECT * FROM sh_berita,sh_users, sh_kategori WHERE sh_berita.s_username=sh_users.s_username AND sh_berita.id_kategori=sh_kategori.id_kategori AND status_terbit='1' AND sh_berita.s_username='$_SESSION[adminsh]' ORDER BY id_berita DESC");
			}
			...
			<tr class="tabel">
				...
				<td class="tabel" width="150px">
					<div class="hapusdata">
					<a href="<?php echo "$database?pilih=terbit&untukdi=hapus&id=$db[id_berita]";?>">hapus</a>
					</div>
					...
					<a href="<?php echo "$database?pilih=terbit&untukdi=terbitkan&id=$db[id_berita]";?>">terbitkan</a>
					</div>
					...
					<div class="bataldata">
					<a href="<?php echo "$database?pilih=terbit&untukdi=batalkan&id=$db[id_berita]";?>">batalkan</a>
					</div>
					<?php }?>
				</td>
			</tr>
			...
			<tr class="tabel"><td class="tabel" colspan="8"><b>DATA BERITA TERBIT BELUM TERSEDIA</b></td></tr>
			<?php }
			?>
