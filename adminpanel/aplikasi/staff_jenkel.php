...
<div class="isikanan"><!--Awal class isi kanan-->
		...
		<table class="tabel" id="results">
			...
			<?php
			$no=1;
			$staff=mysql_query("SELECT * FROM sh_guru_staff, sh_jabatan WHERE sh_guru_staff.id_jabatan=sh_jabatan.id_jabatan AND jenkel='$_GET[kode]' AND posisi='staff' ORDER BY id_gurustaff DESC");
			...
			<tr class="tabel">
				...
				<td class="tabel">
					...
					<?php if ($_GET[kode]=='L'){ ?>
					<div class="hapusdata"><a href="<?php echo "$database?pilih=laki&untukdi=hapus&id=$s[id_gurustaff]";?>">hapus</a></div>
					<?php }
					else { ?>
					<div class="hapusdata"><a href="<?php echo "$database?pilih=perempuan&untukdi=hapus&id=$s[id_gurustaff]";?>">hapus</a></div>
					<?php }?>
			...
			<tr class="tabel"><td class="tabel" colspan="8"><b>DATA STAFF <u>
			<?php
