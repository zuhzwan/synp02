...
<div class="isikanan"><!--Awal class isi kanan-->
		...
		<table class="tabel" id="results">
			...
			$no=1;
			if($_SESSION[leveluser]=='Super Admin'){
			$komentar=mysql_query("SELECT * FROM sh_komentar, sh_berita WHERE sh_komentar.id_berita=sh_berita.id_berita AND status_terima='1' ORDER BY id_komentar DESC");
			}
			else {
			$komentar=mysql_query("SELECT * FROM sh_komentar, sh_berita WHERE sh_komentar.id_berita=sh_berita.id_berita AND status_terima='1' AND sh_berita.s_username='$_SESSION[adminsh]' ORDER BY id_komentar DESC");
			}
			...
			<tr class="tabel">
				...
				<?php 
				echo "
					<div class='hapusdata'><a href='$database?pilih=terima&untukdi=hapus&id=$kom[id_komentar]'>hapus</a></div>
					<div class='editdata'><a href='?pilih=edit&id=$kom[id_komentar]'>edit</a></div>
					<div class='bataldata'><a href='$database?pilih=terima&untukdi=tolak&id=$kom[id_komentar]'>tolak</a></div>";
				?>
				</td>
			</tr>
			<?php
			...
			<tr class="tabel"><td class="tabel" colspan="7"><b>TIDAK ADA DATA KOMENTAR YANG DITERIMA</b></td></tr>
			<?php }
			?>
