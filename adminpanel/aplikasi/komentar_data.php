<?php
$database="aplikasi/database/komentar.php";
switch($_GET['pilih']){
default: ?>
<h3>Komentar</h3>
<div class="isikanan"><!--Awal class isi kanan-->
		...
		<?php echo "<form method='POST' action='$database?pilih=komentar&untukdi=hapusbanyak'>"; ?>
		...
		<table class="tabel" id="results">
			...
			$no=1;
			if($_SESSION[leveluser]=='Super Admin'){
			$komentar=mysql_query("SELECT * FROM sh_komentar, sh_berita WHERE sh_komentar.id_berita=sh_berita.id_berita ORDER BY id_komentar DESC");
			}
			else {
			$komentar=mysql_query("SELECT * FROM sh_komentar, sh_berita WHERE sh_komentar.id_berita=sh_berita.id_berita AND sh_berita.s_username='$_SESSION[adminsh]' ORDER BY id_komentar DESC");
			}
			...
			<tr class="tabel">
				...
				echo "
					<div class='hapusdata'><a href='$database?pilih=komentar&untukdi=hapus&id=$kom[id_komentar]'>hapus</a></div>
					<div class='editdata'><a href='?pilih=edit&id=$kom[id_komentar]'>edit</a></div>
					<div class='bataldata'><a href='$database?pilih=komentar&untukdi=tolak&id=$kom[id_komentar]'>tolak</a></div>"; }
				else {
					echo "
					<div class='hapusdata'><a href='$database?pilih=komentar&untukdi=hapus&id=$kom[id_komentar]'>hapus</a></div>
					<div class='editdata'><a href='?pilih=edit&id=$kom[id_komentar]'>edit</a></div>
					<div class='terbitdata'><a href='$database?pilih=komentar&untukdi=terima&id=$kom[id_komentar]'>terima</a></div>";
				}
					?>
				</td>
			</tr>
...
case "tambah":
include "aplikasi/komentar_tambah.php";
break;

case "edit":
include "aplikasi/komentar_edit.php";
break;

case "diterima":
include "aplikasi/komentar_terima.php";
break;

case "ditolak":
include "aplikasi/komentar_tolak.php";
break;

case "pencarian":
include "aplikasi/komentar_pencarian.php";
